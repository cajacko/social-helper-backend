...
CREATE TABLE IF NOT EXISTS `socialhelper`.`trackingTypes` (
  `trackingTypeID` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `type` VARCHAR(45) NOT NULL,
  `dateAdded` DATETIME NOT NULL,
  `dateUpdated` DATETIME NOT NULL,
  PRIMARY KEY (`trackingTypeID`),
  UNIQUE INDEX `trackingTagsID_UNIQUE` (`trackingTypeID` ASC),
...
CREATE TABLE IF NOT EXISTS `socialhelper`.`objectTrackingQuery` (
  ...
  INDEX `objectTrackingQueryAccountTrackingQueryID_idx` (`trackingQueryID` ASC),
...
CREATE TABLE IF NOT EXISTS `socialhelper`.`trackingQueryMeta` (
  `trackingQueryMetaID` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `trackingQueryID` INT UNSIGNED NOT NULL,
  `metaKey` VARCHAR(45) NOT NULL,
  `metaValue` VARCHAR(45) NULL,
  `dateAdded` DATETIME NOT NULL,
  `dateUpdated` DATETIME NOT NULL,
  PRIMARY KEY (`trackingQueryMetaID`),
  UNIQUE INDEX `trackingQueryMetaID_UNIQUE` (`trackingQueryMetaID` ASC),
  UNIQUE INDEX `trackingQueryIDmetaKey` (`trackingQueryID` ASC, `metaKey` ASC),
